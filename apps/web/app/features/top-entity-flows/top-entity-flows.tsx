...
export function TopEntityFlows({ initialFlows }: TopEntityFlowsProps) {
  ...
  const mode = normalizeTopEntityFlowMode(
    searchParams.get('mode') ?? initialFlows.meta.mode,
  );
  const windowMinutes = normalizeTopEntityFlowWindow(
    searchParams.get('windowMinutes') ??
      initialFlows.meta.window.minutes.toString(),
  );
  ...
  const flowsQuery = useQuery({
    ...
    initialDataUpdatedAt:
      initialData === undefined
        ? undefined
        : Date.parse(initialData.meta.generatedAt),
    placeholderData: keepPreviousData,
    queryFn: ({ signal }) =>
      fetchTopEntityFlows({ mode, signal, windowMinutes }),
  ...
  return (
    <Panel>
      ...
      <div
        className={
          isRefreshing
            ? 'divide-y divide-border opacity-75'
